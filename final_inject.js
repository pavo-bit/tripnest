...
['index.html', 'trips.html', 'destinations.html'].forEach(filename => {
    ...
    // Restore head & local assets
    headReplacements.forEach(r => content = content.replace(new RegExp(r.from), r.to));
    ...
    if (filename === 'index.html') {
        ...
        if (content.includes(lastTripFoot)) {
            ...
            content = parts[0] + lastTripFoot + '\n' + cardsHTML + parts.slice(1).join(lastTripFoot);
        }
    } else if (filename === 'destinations.html') {
    ...
    // IMPORTANT: Inject panels inside #trip-detail-panel precisely.
    // The panel container ALWAYS ends with id="content-chopta" natively.
    // By splitting on 'id="content-chopta"', we find the exact closing tag of #trip-detail-panel 
    // without messing up any siblings.
    if(content.includes('id="content-chopta"')) {
        let parts = content.split('id="content-chopta"');
        parts[1] = parts[1].replace('</section>', panelsHTML + '\n    </section>');
        content = parts[0] + 'id="content-chopta"' + parts[1];
    } else {
        console.error("FAIL: NO chopta content in", filename);
    }

    fs.writeFileSync(filename, content, 'utf8');
    console.log("Re-rendered perfectly: " + filename);
});
