...
['index.html', 'trips.html', 'destinations.html'].forEach(filename => {
    ...
    // Safety check - avoid duplicate injections if we re-run
    if (!content.includes('id="content-tungnath"')) {
        // Find the end of Chopta content explicitly, or any section right before CTA
        let parts = content.split(/(\s*<\/div>\s*<\/div>\s*<\/div>\s*)(<\/section>\s*<!\-\-\s*(REVIEWS|GALLERY|ABOUT US|CTA)\s*\-\->)/);
        
        if (parts.length > 2) {
            // parts[0] is everything BEFORE the closing tags
            // parts[1] is the closing tags of the last #content-chopta div
            // parts[2] is </section> plus the next HTML comment
            content = parts[0] + parts[1] + panelsHTML + '\n    ' + parts[2];
            console.log("Successfully injected panels into " + filename);
        } else {
            console.log("Could not find insertion point via regex in " + filename);
            // Fallback for trips.html / destinations.html where structure might differ
            let fbParts = content.split('</section>');
            // find the one containing id="content-chopta" or another content pane
            let inserted = false;
            for(let i=0; i<fbParts.length; i++){
                if(fbParts[i].includes('id="content-')) {
                   fbParts[i] = fbParts[i] + panelsHTML;
                   content = fbParts.join('</section>');
                   inserted = true;
                   console.log("Fallback injected into " + filename);
                   break;
                }
            }
            if(!inserted) console.log("Failed completely to inject panels into " + filename);
        }
        
        fs.writeFileSync(filename, content, 'utf8');
    } else {
        console.log("Panels already exist in " + filename);
    }
});
