...
const tripsSectionBase = `
    <section id="trips" class="section page-header-gap">
      <div class="container">
        <div class="sec-head text-center reveal">
          <div class="badge badge-ol">✦ Handpicked for You</div>
          <h2>Popular Trips</h2>
          <div class="rule"></div>
          <p>
            Curated adventures — from sacred Himalayan shrines to tropical shores.
          </p>
        </div>
...
<!-- CARDS_HERE -->
        ...
        <div class="text-center" style="margin-top: 40px">
          <a href="/trips.html" class="btn btn-outline">View All Trips <i class="fas fa-arrow-right"></i></a>
        </div>
      </div>
    </section>
`;
          ...
          <div class="dd-grid">
            <div>
              ...
              <div class="inc-box" style="background: var(--white); border: 1px solid rgba(90, 122, 50, 0.18); border-radius: var(--r-md); padding: 20px 22px; margin-bottom: 18px;">
                ...
                <div style="font-family: 'Cormorant Garamond', serif; font-size: 1rem; font-weight: 700; color: var(--ol-darkest); margin-top: 20px; margin-bottom: 12px;">❌ Not Included</div>
                <div style="display: grid; grid-template-columns: 1fr; gap: 10px">
                  <div class="inc-item"><i class="fas fa-times-circle" style="color: #d9534f"></i><div><h4>Pony / Doli / Helicopter</h4></div></div>
                  <div class="inc-item"><i class="fas fa-times-circle" style="color: #d9534f"></i><div><h4>VIP Darshan / Entry fees</h4></div></div>
                </div>
              </div>
              <a href="https://wa.me/917388594640" target="_blank" class="btn btn-wa" style="width: 100%; justify-content: center"><i class="fab fa-whatsapp" style="font-size: 1.1rem"></i> Book Now on WhatsApp</a>
            </div>
...
function doInject() {
  ['index.html', 'trips.html', 'destinations.html'].forEach(filename => {
    ...
    // For index.html, if it has NO trips-grid, we must inject tripsSectionBase replacing <!-- POPULAR TRIPS -->
    if (filename === 'index.html') {
      // trips.html has trips-grid, but index.html maybe has <!-- POPULAR TRIPS --> and then <!-- WHY CHOOSE US --> directly.
      if (!content.includes('class="trips-grid"')) {
         // get the trips HTML we want (just cardsHTML without any old cards, but wait, the prompt asks to add the cards to the Section)
         // So if there are no old cards in index.html, we just put the two new cards.
         const sectionHtml = tripsSectionBase.replace('<!-- CARDS_HERE -->', cardsHTML);
         content = content.replace('<!-- POPULAR TRIPS -->', '<!-- POPULAR TRIPS -->\n' + sectionHtml);
      } else {
         // Append before nearest </div> closure of trips-grid
         content = content.replace(/(class="trips-grid"[\s\S]*?)(<\/div>\s+)(?:<\/div>\s+)?<\!-- WHY CHOOSE US -->|<div class="text-center"/, (match, p1, p2) => {
            return p1 + cardsHTML + p2;
         });
         // The regex might be tricky, let's use exact string matching for safety below.
      }
    } else {
      // In trips.html and destinations.html, we KNOW class="trips-grid" exists.
      // We find the FIRST closing </div> of trips-grid.
      // Actually, since trips-grid is immediately followed by </div></section>, or a view all button.
      // Let's just find "        </div>\n      </div>\n    </section>" in trips.html/destinations.html
      // wait, trips.html has `<div class="text-center" style="margin-top: 40px">` inside it? No, in trips.html there is no view all.
      // Let's do a reliable replacement: find the last trip card, and append after it.
      const lastTripFoot = `</button>\n              </div>\n            </div>\n          </div>`;
      if (content.includes(lastTripFoot)) {
          let parts = content.split(lastTripFoot);
          // Split by lastTripFoot. Re-assemble:
          content = parts[0] + lastTripFoot + cardsHTML + parts.slice(1).join(lastTripFoot);
      } else {
          // Alternatively, replace \n        </div>\n      </div>\n    </section>
          content = content.replace(/\n\s*<\/div>\s*<\/div>\s*<\/section>\s*<\!-- WHY CHOOSE US -->/, (match) => {
               return '\n' + cardsHTML + match;
          });
      }
    }

    // Append the panels right before the closing </section> of trip-detail-panel
    // The panel closes right before <!-- WHY CHOOSE US --> OR <!-- DESTINATIONS --> OR an EOF.
    // In index.html, trip-detail-panel closes right before <footer> maybe? No, before `<!-- FOOTER -->` or something?
    // Let's find '</section>\n\n    <!-- FOOTER -->' or '</section>\n    <!-- FOOTER -->'
    if (content.includes('</section>\n\n    <!-- FOOTER -->')) {
        ...
        // Last resort
        content = content.replace('</body>', panelsHTML + '\n</body>');
    }
...
doInject();
console.log("Successfully injected safely.");
